...
def is_accepted_bfs(init_node, tr, adjlst, debug=False):
    ...
    for current_index in xrange(len(tr)):
...
class StandardAutomata:
    ...
    def nfa2dfa(self):
        ...
        state_mapping = {state_index[i]: str(i) for i in xrange(len(found_states))}

        g = StandardAutomata(map(lambda x: state_mapping[x], starting_states),
                             map(lambda x: (state_mapping[x[0]], state_mapping[x[1]], x[2]), dfa_transitions),
                             map(lambda x: state_mapping[x], ending_states))
        return g
    ...
    def minimize_dfa(self):
        ...
        min_startings = map(lambda x: element2cluster[x], self.startings)
        min_endings = map(lambda x: element2cluster[x], self.endings)
